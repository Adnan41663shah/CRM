...
import { X, FileText, Clock, Mail, Phone, MapPin, BookOpen, Calendar, Eye } from 'lucide-react';
...
interface PresalesUserDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  userId: string;
}

interface PresalesUserDetails {
  user: {
    _id: string;
    name: string;
    email: string;
...
const PresalesUserDetailsModal: React.FC<PresalesUserDetailsModalProps> = ({
  isOpen,
  onClose,
  userId,
}) => {
  ...
  const { data, isLoading, error } = useQuery(
    ['presales-user-details', userId],
    () => apiService.inquiries.getPresalesUserDetails(userId),
    {
  ...
  const userDetails: PresalesUserDetails | undefined = data?.data;

  // Filtered Data Logic
  const filteredInquiries = React.useMemo(() => {
    if (!userDetails?.createdInquiries) return [];
    
    return userDetails.createdInquiries.filter(inquiry => {
      if (inquiryFilter === 'all') return true;
      
      const dateToCheck = inquiry.createdAt;
  ...
  }, [userDetails?.createdInquiries, inquiryFilter, inquiryCustomRange]);
        ...
        <>
          ...
          >
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-6xl max-h-[95vh] sm:max-h-[90vh] flex flex-col border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between border-t border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 sm:px-6 shadow-sm z-10">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="hidden sm:inline">Created Inquiries</span>
                      ...
                      <span className={cn(
                        ...
                        {filteredInquiries.length} / {userDetails?.createdInquiries.length || 0}
                      </span>
                    </div>
                    {activeTab === 'inquiries' && (
                      <motion.div
              ...
              <div className="flex-1 overflow-y-auto p-2 sm:p-3 md:p-4 bg-gray-50/50 dark:bg-gray-950/50">
                {isLoading ? (
                    ...
                    {filteredInquiries.length === 0 ? (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
                          {inquiryFilter === 'today' ? 'No inquiries created today' : inquiryFilter === 'custom' ? 'No inquiries found in the selected date range' : 'No inquiries created by this user'}
                        </p>
                      </motion.div>
                    ) : (
                        ...
                        <div className="overflow-x-auto">
                          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-linear-to-r from-gray-50 to-gray-100/50 dark:from-gray-800 dark:to-gray-800/50">
                              <tr>
                                <th className="px-2 sm:px-3 md:px-4 py-1.5 text-left text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                  Name
                                </th>
                                <th className="px-2 sm:px-3 md:px-4 py-1.5 text-left text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                  Phone
                                </th>
                                <th className="px-2 sm:px-3 md:px-4 py-1.5 text-left text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider hidden md:table-cell">
                                  Course
                                </th>
                                <th className="px-2 sm:px-3 md:px-4 py-1.5 text-left text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider hidden lg:table-cell">
                                  Location
                                </th>
                                <th className="px-2 sm:px-3 md:px-4 py-1.5 text-left text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                  Status
                                </th>
                                <th className="px-2 sm:px-3 md:px-4 py-1.5 text-left text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider hidden xl:table-cell">
                                  Created
                                </th>
                                <th className="px-2 sm:px-3 md:px-4 py-1.5 text-center text-[10px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                  Action
                                </th>
                              </tr>
                            </thead>
                            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                              {filteredInquiries.map((inquiry, index) => (
                                ...
                                >
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5">
                                    <div className="text-[11px] sm:text-xs font-semibold text-gray-900 dark:text-white truncate max-w-[140px] sm:max-w-none">
                                      {inquiry.name || '-'}
                                    </div>
                                    <div className="text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400 sm:hidden mt-0.5 truncate flex items-center gap-1">
                                      <Mail className="h-2.5 w-2.5" />
                                      {inquiry.email || '-'}
                                    </div>
                                  </td>
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5">
                                    <div className="flex items-center gap-1.5 text-[11px] sm:text-xs text-gray-600 dark:text-gray-400">
                                      <Phone className="h-3 w-3 shrink-0 text-gray-400 dark:text-gray-500" />
                                      <span className="truncate font-mono">{inquiry.phone || '-'}</span>
                                    </div>
                                  </td>
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5 hidden md:table-cell">
                                    <div className="flex items-center gap-1.5 text-[11px] sm:text-xs text-gray-600 dark:text-gray-400">
                                      <BookOpen className="h-3 w-3 shrink-0 text-gray-400 dark:text-gray-500" />
                                      <span className="truncate">{inquiry.course || '-'}</span>
                                    </div>
                                  </td>
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5 hidden lg:table-cell">
                                    <div className="flex items-center gap-1.5 text-[11px] sm:text-xs text-gray-600 dark:text-gray-400">
                                      <MapPin className="h-3 w-3 shrink-0 text-gray-400 dark:text-gray-500" />
                                  ...
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5">
                                    <span
                                      className={cn(
                                        'inline-flex items-center px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-semibold shadow-sm',
                                  ...
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5 hidden xl:table-cell">
                                    <div className="flex items-center gap-1 text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400">
                              ...
                              {filteredFollowups.map((followup, index) => (
                                ...
                                >
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5">
                                    ...
                                    {followup.title && (
                                      <div className="text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400 sm:hidden mt-0.5">
                                        {followup.title}
                                      </div>
                                    )}
                                  </td>
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5">
                                    <div className="flex items-center gap-1.5 text-[11px] sm:text-xs text-gray-600 dark:text-gray-400">
                                      <Phone className="h-3 w-3 shrink-0 text-gray-400 dark:text-gray-500" />
                                      <span className="truncate font-mono">{followup.inquiry.phone || '-'}</span>
                                    </div>
                                  </td>
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5 hidden md:table-cell">
                                    <div className="flex items-center gap-1.5 text-[11px] sm:text-xs text-gray-600 dark:text-gray-400">
                                  ...
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5 hidden xl:table-cell">
                                    <div className="text-[11px] sm:text-xs text-gray-600 dark:text-gray-400 max-w-[180px] truncate">
                                      {followup.message || '-'}
                                    </div>
                                  </td>
                                  <td className="px-2 sm:px-3 md:px-4 py-1.5 hidden lg:table-cell">
...
export default PresalesUserDetailsModal;
