...
import { Search, Download, GraduationCap, Filter, ChevronDown, X } from 'lucide-react';
...
const AdmittedStudents: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setViewMode(searchParams.get('tab') === 'all' ? 'all' : 'my');
  }, [searchParams]);

  useEffect(() => {
    const params = inquiryFiltersToParams(filters, currentPage);
    if (viewMode === 'all') params['tab'] = 'all';
  ...
  // All admitted students should be shown here regardless of unattended status
  ...
  // Get admission date as Date object for filtering
  const getAdmissionDateObject = (inquiry: Inquiry): Date | null => {
    if (!inquiry.followUps || inquiry.followUps.length === 0) {
      return null;
    }
    ...
    return new Date(latestFollowUp.createdAt);
  };
  ...
  const viewModeStudents = useMemo(() => {
    if (viewMode === 'all') return admittedStudents;
    return admittedStudents.filter((inquiry: Inquiry) => 
      inquiry.assignedTo?.id === user?.id || (inquiry.assignedTo as any)?._id === user?.id
    );
  }, [admittedStudents, viewMode, user]);

  // Apply search filter
  const filteredStudents = useMemo(() => {
    if (!filters.search) {
      return viewModeStudents;
    }
    
    const searchLower = filters.search.toLowerCase();
    return viewModeStudents.filter((inquiry: Inquiry) => {
      return (
  ...
  }, [viewModeStudents, filters.search]);
  ...
  const finalFilteredStudents = useMemo(() => {
    ...
    // Apply date filters based on admission date (when the "Confirmed Admission" follow-up was created)
    if (filters.dateFrom) {
      const fromDate = new Date(filters.dateFrom);
      fromDate.setHours(0, 0, 0, 0);
      filtered = filtered.filter((inquiry: Inquiry) => {
        const admissionDate = getAdmissionDateObject(inquiry);
        if (!admissionDate) {
          return false;
        }
    ...
    return filtered;
  }, [filteredStudents, filters.course, filters.location, filters.dateFrom, filters.dateTo]);

  // Client-side pagination
  const totalPages = Math.ceil(finalFilteredStudents.length / ITEMS_PER_PAGE);
  const paginatedStudents = useMemo(() => {
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;
    return finalFilteredStudents.slice(startIndex, endIndex);
  }, [finalFilteredStudents, currentPage]);
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      {isLoading ? (
        <div className="flex items-center justify-center min-h-64 sm:min-h-80">
          <LoadingSpinner size="lg" label="Loading admitted students..." />
        </div>
...
{/* Action column removed */}
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                  {paginatedStudents.map((inquiry: Inquiry, index: number) => (
                    <motion.tr
                      key={inquiry._id}
          ...
          {finalFilteredStudents.length > 0 && (
            <div className="flex items-center justify-between mt-2 p-2 border-t border-gray-200 dark:border-gray-700">
              ...
              <Pagination
                ...
                totalItems={finalFilteredStudents.length}
