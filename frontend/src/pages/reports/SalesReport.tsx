...
import SalesUserDetailsModal from '@/components/SalesUserDetailsModal';
import { cn } from '@/utils/cn';

const MODAL_STATE_KEY = 'sales-report-modal-state';

interface SalesUserStats {
  userId: string;
  name: string;
  email: string;
  totalInquiriesAttended: number;
  totalConvertedToAdmissions: number;
...
const SalesReport: React.FC<SalesReportProps> = ({ filterParams = {}, searchTerm }) => {
        ...
        if (saved) {
          try {
            ...
            if (parsed.isOpen && parsed.userId) {
              ...
              setIsModalOpen(true);
              // Also ensure the sales tab is active in the Reports page
              const reportsTab = sessionStorage.getItem('reports-active-tab');
              if (reportsTab !== 'sales') {
                sessionStorage.setItem('reports-active-tab', 'sales');
              }
            }
          } catch {
            // Ignore parse errors
  ...
  const { data, isLoading, error, refetch } = useQuery(
    ['sales-report', filterParams],
    () => apiService.inquiries.getSalesReport(filterParams),
    {
  ...
  const salesUsers: SalesUserStats[] = React.useMemo(() => {
    const users = data?.data?.users || [];
    const term = (searchTerm || '').trim().toLowerCase();

    const filtered = term
      ? users.filter((user: SalesUserStats) =>
                ...
                salesUsers.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-1.5 text-center text-sm text-gray-900 dark:text-white">
                      {user.totalInquiriesAttended}
                    </td>
                    <td className="px-4 py-1.5 text-center">
                      <span className={cn(
                        "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium",
                        user.totalConversions > 0
                    ...
                    <td className="px-4 py-1.5 text-center text-sm text-gray-900 dark:text-white">
                      {user.totalConvertedToAdmissions}
                    </td>
                    <td className="px-4 py-1.5 text-center">
                      <span className={cn(
                        "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium",
